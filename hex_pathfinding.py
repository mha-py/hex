...
def own_path_finding(stones):
    '''My own version of a path finding algorithm. Can be used to test against.
    stones: nxn-array of bool type, where the stones of the player are place.
    Time: ~139 µs
    '''
    Nx, Ny = stones.shape
            ...
            for j in range(N):
                if con_new[i,j]:
                    ...
                    for (di, dj) in [(1,0), (0,1), (-1,1), (-1,0), (0,-1), (1,-1)]:
                        if 0<=i+di<N and 0<=j+dj<N:
                            if not connected[i+di,j+dj] and stones[i+di,j+dj]==1:
            ...
            if 0<=i+di<N and 0<=j+dj<N:
                if stones[i+di, j+dj] == 1:
                    ...
                    for expld in explored:
                        append = (append and (expld != (i+di, j+dj)))
                    if append:
...
def voltage(board):
    'Heuristical map that quantifies the distance of a field to the borders: -1 is directly connected to the left edge, +1 directly to the right edge'
